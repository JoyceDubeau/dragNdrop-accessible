...
var drag = {
	...
	arTargets : ['dropzone1', 'dropzone2', 'dropzone3', 'dropzone4', 'dropzone5' ],
	...
	keyboardDragDrop : function(objEvent)
	{
		...
		var arChoices = ['dropzone1', 'dropzone2', 'dropzone3', 'dropzone4', 'dropzone5'];
	...
	dropObject : function(strTarget)
	{
		...
		var objClone, objOriginal, objTarget, objEmpty, objBands, objItem, strAnswer;
		...
		if (strTarget.length > 0 && strTarget == drag.strAnswer) //Added "&& strTarget == drag.strAnswer" for validation of the right answer. The drag object will only drop in the right dropzone.
		{
			...
			if (objBands.length === 0)
			{
				...
				objItem.appendChild(document.createTextNode('\xa0'));
	...
	{
		console.log("strAnswer in end = " + drag.strAnswer);
	...
	{
		var arrAnswers = document.getElementsByName("dropzone"); //gets all the dropzones
		console.log("arrAnswers length = " + arrAnswers.length);
		console.log("arrAnswers = " + arrAnswers);
		
		var sumAnswers = 0;
		var iCounter = 0;
		
		for (iCounter=0; iCounter<arrAnswers.length; iCounter++)
		{
			console.log("arrAnswers[iCounter] = " + arrAnswers[iCounter]);
			if (arrAnswers[iCounter].getAttribute("data-answered") == "true") //checks how many dropzones are filled
			{
				sumAnswers++;
				console.log("sumAnswers ++ (true) = " + sumAnswers);
			}
		}
		
		if (sumAnswers === arrAnswers.length) //if the total of filled dropzones is the same as the total of dropzones
		{
			console.log("sumAnswers EQUAL arrAnswers.length");
			document.getElementById("feedback-container").style.display = "block"; //triggers the feedback
		}
		else
		{
			console.log("sumAnswers NOT EQUAL arrAnswers.length");
		}
	}
};
